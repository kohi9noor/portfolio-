...
const menuItems = [
  { icon: HomeIcon, label: "Homepage", size: 20 },
  { icon: BriefcaseConveyorBelt, label: "Projects", size: 20 },
  { icon: UserIcon, label: "About", size: 20 },
  { icon: PencilIcon, label: "Blog", size: 20 },
  { icon: MailIcon, label: "Contact", size: 20 },
];
    ...
    <React.Fragment>
      <div className="flex flex-col items-center gap-6">
        ...
        <div className="flex flex-col gap-1 items-center w-full">
          {menuItems.map(({ icon: Icon, label, size }, index) => (
            <TooltipWrapper key={index} text={label}>
              <div className="flex items-center md:justify-center 2xl:justify-start md:w-12 2xl:w-[240px] 2xl:px-4 2xl:h-10 gap-4 cursor-pointer md:h-12 rounded-2xl xl:rounded-xl hover:bg-slate-200">
                <Icon size={size} />
        ...
        <div className="flex flex-col gap-2 items-center">
          {socialLinks.map(({ icon: Icon, url, size }, index) => (
            <a key={index} href={url} target="_blank" rel="noopener noreferrer">
              <div className="flex items-center justify-center rounded-full w-12 h-12 bg-slate-200 hover:bg-slate-300">
                <Icon size={size} />
              </div>
            </a>
          ))}
          <div className="flex items-center justify-center w-12 cursor-pointer h-12 rounded-full hover:bg-slate-200">
            <MoonIcon size={18} />
          </div>
        </div>
      </div>
